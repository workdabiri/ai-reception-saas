...
| 2026-06-30 | ACCEPT | P12-B accepted as docs-only schema/domain design contract (`docs/audits/AREA-C-p12b-schema-domain-design.md`): `WebChatChannelBinding` + `WebChatChannelBindingStatus {ACTIVE, REVOKED}`; `allowedOrigins String[]` (alpha); widget-key plaintext never stored — keyed hash (HMAC-SHA-256 / server-side peppered), raw key shown once at creation/rotation, immediate rotation (no previous-key grace window); `channels.read`/`channels.manage` OWNER+ADMIN only, OPERATOR+VIEWER denied both, `channels.manage` sensitive/audit-required; implementation remains STOP-for-approval (no code/schema/migration/route/widget/provider/AI go-live/auto-send) | — | low (docs) |
