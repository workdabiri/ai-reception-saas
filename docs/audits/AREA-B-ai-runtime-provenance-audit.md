...
| Can AI auto-send to customers today? | **No** — structurally no send path exists (see §3, Q13). [VERIFIED] |
...
**Q13 — Is the system structurally unable to auto-send AI replies?** **Yes, currently — with one forward-scaffolding caveat.**
- No send endpoint, no `sendDraft` repository method, and **no code transitions a draft to `SENT`** anywhere in `src`. `approveDraft` is documented and implemented to "**Does NOT create a Message. Does NOT call any provider.**" The repository `update` type cannot even write `sentMessageId` / `sentAt` / `sentByUserId`. [VERIFIED: `reply-drafts/repository.ts:213`, `:113`]
- **Caveat:** the RBAC catalog **defines and grants** `ai_drafts.send` (to OPERATOR, and it is in `SENSITIVE_PERMISSIONS`), but **no handler consumes it** — it is forward-scaffolding for a future human-initiated send path. The risk to guard later: when a send path is built, it must be **human-initiated only** and must never be wired to an automatic trigger. [VERIFIED: `authz/permissions.ts:39,64`]
...
| B-9 | `ai_drafts.send` granted but no send path — future send path could be wired to auto-trigger | Forward-scaffolding; currently inert | Medium | **SHOULD_DO_BEFORE_ALPHA** | `authz/permissions.ts:39,64`; no consumer | Add a guard/test asserting send is human-initiated only and never auto-invoked; preserve no-auto-send |
