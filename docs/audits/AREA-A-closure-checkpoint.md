...
| Stronger route-backstop enforcement — AST/ESLint rule or typed route wrapper instead of the meta-test convention | (A-H4 hardening) | **Closed (PR #118, merge commit `8b71fd3`).** AST/static guard now enforces the structural chokepoint against future bypass regressions |

None of these block the first single real-business backend gate; they harden the path to multi-tenant scale.

**Update — Concurrent multi-tenant isolation tests: completed (PR #115, merge commit `b9a0eb3`).** The previously-open concurrent multi-tenant isolation item (A-R1 extension) — proving parallel requests do not bleed across tenants via connection pool or cache — is now **closed**: a real-DB concurrent tenant-isolation suite was merged in PR #115 and the real-DB **Tenant Isolation Integration** (A-R1) CI gate passed. This was a pre-scale hardening item, not a blocker to the Area A backend GREEN verdict. At the time of PR #115, the remaining non-blocking hardening items were A-H1 composite FK hardening, A-H2 assignee-is-member check, and the route-backstop hardening; the route-backstop hardening was **later closed by PR #118 (merge commit `8b71fd3`)**, while **A-H1 and A-H2 remain open by design**. This update closes one non-blocking hardening item only; it does not declare all Area A future hardening complete, and it does not change the §3 verdict or lift the §5 conditional blockers (Area B / AI, Area C / widget).

**Update — Route-backstop AST/static hardening (A-H4): completed (PR #118, merge commit `8b71fd3`).** The previously-open route-backstop hardening item — replacing the meta-test convention with a stronger AST/static guard against future bypass regressions — is now **closed**: PR #118 added an AST guard helper (`__tests__/_helpers/tenant-route-guard-ast.ts`) and a consolidation test (`__tests__/foundation/tenant-route-guard-consolidation.test.ts`), and both the `CI/Lint, Typecheck, Build, Test` and the real-DB `Tenant Isolation Integration` (A-R1) CI gates passed. This was a pre-scale hardening item, not a blocker to the Area A backend GREEN verdict. **Only the A-H4 route-backstop hardening item is closed by this update.** The remaining items above — **A-H1 composite FK hardening (Open)** and **A-H2 assignee-is-member check (Open / deferred by design)** — **remain open by design**. This update closes one non-blocking hardening item only; it does not declare all Area A future hardening complete, and it does not change the §3 verdict or lift the §5 conditional blockers (Area B / AI, Area C / widget).
