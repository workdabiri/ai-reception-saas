...
> **Status note — prompt-injection / untrusted-input trust-boundary work (2026-06-20).** Beyond the `B-R*` / `B-H*` tasks tabulated below, the checkpoint §6 **"Prompt-injection / untrusted user-message strategy"** gate has advanced as follows: the vendor-neutral trust model and re-proof obligations were **specified** in `docs/audits/AREA-B-prompt-injection-untrusted-input-strategy.md` (PR #130), and the **test-only trust-boundary enforcement step is now COMPLETED / MERGED** as **PR #131 / commit `f985219`** — a test-only trust-boundary policy helper (`__tests__/_helpers/ai-runtime-trust-boundary-policy.ts`) and suite (`__tests__/domains/ai-runtime-trust-boundary.test.ts`) proving, for the **current no-untrusted-content scope**, TRUSTED / SEMI_TRUSTED / UNTRUSTED classification, default-deny, untrusted customer/conversation/widget-shaped input **blocked from prompt construction**, semi-trusted overreach denied, and the draft-only / human-review / no-auto-send / verified-context-only / no-leak rules kept visible — with the helper scope free of provider SDKs, network, env/API-key, DB, send, and customer-read paths. This added **no production code, no real provider, no SDK, no env/API-key read, no route-level wiring, no production prompt-builder change, no schema/migration, and no auto-send**, and **B-R7 / B-R8 remained green**. **This does NOT close the prompt-injection / untrusted-input gate**: it proves the *current* boundary only. **Future work is still required before any customer message can enter prompts** — per the strategy doc §6, that means a fenced untrusted-content region, a no-authority-from-untrusted rule, re-proof of data-minimization / no-auto-send / tenant-isolation, adversarial prompt-injection regression tests, and an audit/privacy review, all under explicit owner approval and a dedicated PR. **Customer-message-in-prompt remains STOP; the gate remains OPEN; real-provider production AI-assisted go-live remains NOT YET APPROVED.**

> **Status note — kill-switch / rollback drill runbook DOCUMENTED (PR #133, 2026-06-20).** The §7 "Kill switch documented" operational requirement is now satisfied at the **documentation tier**: PR #133 / commit `b1207ae` added a **docs-only** Area B AI-runtime kill-switch / rollback drill runbook (`docs/operations/ai-runtime-kill-switch-rollback-drill-runbook.md`) recording where the per-business kill switch lives (`Business.aiMode` → revert to `MANUAL`), how to flip it **without a deploy**, the **fail-closed guarantee** (`AI_DISABLED`, no draft, no provider call), a drill checklist, and a **blank owner sign-off template**, citing existing green tests as acceptance evidence (`__tests__/api/reply-draft-generate-handler.test.ts`, the B-R1 resolver suite, B-R7, B-R8). The checkpoint §6 / real-provider-readiness-gate.md gate #11 (rollback / kill-switch drill) is therefore **ADVANCED, NOT CLOSED**: the **witnessed rehearsal (execution)** and the **recorded owner sign-off** remain **future work**. This added **no production code, no real provider, no SDK, no env/API-key read, no route-level wiring, no schema/migration, no DB counters, and no auto-send**, and **B-R7 / B-R8 remained green** (not edited). **The gate is not closed; real-provider production AI-assisted go-live remains NOT YET APPROVED.**

> **Status note — AI generation / audit-event logging review COMPLETED / MERGED (PR #135, 2026-06-21).** The audit-logging review step is now **completed/merged**: PR #135 / commit `0659011` added a **review-only, docs-only** Area B review (`docs/audits/AREA-B-ai-generation-audit-logging-review.md`) recording that the reply-draft **generate route emits no general `AuditEvent`** (while **approve / edit / discard do**), the generate handler test has **no audit assertion**, and the **B-R6 `AiGenerationAuditLog` generation-attempt audit is wired to no real route** today. This is **review-only / docs-only**; it does **not** mark B-R6 or route-level audit wiring as implemented, and it changed no production code, test, schema, package, CI, env, provider, or auto-send. **Future owner-approved production-code / test PRs are still required** to: (1) add any general `AuditEvent` to the generate route (a separate, owner-approved production-code PR — not done); (2) wire **B-R6 `AiGenerationAuditLog` to any real generation route** (gated; only with real route-level generation under the real-provider readiness gate); and (3) **prove** the audit logs remain **metadata-only** and exclude customer content, prompt text, provider response text, and secrets. **B-R6 / route audit wiring is NOT implemented; real-provider production AI-assisted go-live remains NOT YET APPROVED.**

> **Status note — generate-route general `AuditEvent` parity IMPLEMENTED / MERGED (PR #137, 2026-06-21).** The general-`AuditEvent` parity implementation step recommended by the PR #135 review is now **completed/merged**: PR #137 / commit `a3d4b3a` added a metadata-only / PII-safe general `AuditEvent` (`action: ai_draft.generated`) to the **current deterministic SYSTEM-stub** reply-draft generate route, emitted **only on successful** stub create/reuse, best-effort, with no emission on disabled / authz / failed paths, proven by test (field allowlist + no-content-leakage + a static guard that no B-R6 audit is wired). This is **scoped to the general `AuditEvent` for the deterministic stub route only** — it is **IMPLEMENTED / CLOSED for the current deterministic SYSTEM-stub route**; it does **not** mark **B-R6 `AiGenerationAuditLog` route wiring** as implemented (`src/domains/ai-runtime/**`, including `audit-log.ts`, was untouched), and it added no real provider, SDK, env/API-key work, route-level real generation, schema/migration, auto-send, or customer-message-in-prompt. **Future owner-approved work is STILL OPEN / still required** to: (1) **wire B-R6 `AiGenerationAuditLog` to any real generation route**; (2) **prove provider/runtime audit metadata for real generation** (metadata-only, excluding customer content, prompt text, provider response text, and secrets); and (3) any **real-provider / route-level generation** work — all gated under the real-provider readiness gate. **B-R6 route wiring is NOT implemented; real-provider production AI-assisted go-live remains NOT YET APPROVED.**

> **Status note — human approval flow verification COMPLETED / MERGED (PR #139, 2026-06-22).** The B-R8-adjacent human-approval-boundary verification step is now **completed/merged** at the **backend handler tier**: PR #139 / commit `620c50e` added a **test-only** safety-gate proof (`__tests__/api/reply-draft-human-approval-flow.test.ts`) for the **current deterministic reply-draft handler boundary**, proving **generate/edit/discard are draft-only/non-send** (producing `PENDING_REVIEW` / `EDITED` / `DISCARDED`, never `APPROVED`/`SENT`), **approve is the explicit authenticated/authorized human approval boundary but is not a send operation** (marks `APPROVED`; creates no outbound/customer-facing message), approval is **never implicit** from generate/edit/discard, and **static guards** confirm the reply-draft handlers/routes carry **no** outbound/send/channel/provider/env/B-R6/auto-send wiring. This is **test-only**; it added **no production source change, no route behavior change, no real provider, no SDK, no env/API-key work, no B-R6 `AiGenerationAuditLog` route wiring, no customer-message-in-prompt, no auto-send, no outbound message creation, and no schema/migration**, and **B-R7 / B-R8 remained green** (not edited). **This does NOT mark B-R6 route wiring implemented, does NOT approve auto-send or a real provider, and does NOT claim go-live readiness:** it proves the *current* deterministic handler boundary only. The full human-approval **UX** gate (end-to-end against real route-level generation) and the remaining §6 / real-provider-readiness gates stay open. **Auto-send is NOT PRESENT / NOT APPROVED; real-provider production AI-assisted go-live remains NOT YET APPROVED.**
...
| B-R6 | AI generation audit log + draft metadata | **BLOCKER** — *token/usage cost-guard decision contract additionally test+doc-proven over the recorded usage counts for the current fake-provider scope (PR #128); see the B-R6 entry below* |
...
- **Status (2026-06-20).** B-R6 is **CLOSED** (PR #103 / `cb0d00f`). Its audit `FAILED` lifecycle is now additionally exercised by an **operational-failure proof**: **PR #126 / `2823e88`** drives provider operational failures (timeout / rate_limited / unavailable / content_filtered / unknown, via a test-only fault provider) through `completeFailure(...)` and proves exactly one metadata-only `FAILED` row per failed attempt — tenant-scoped by `businessId`, terminal-immutable, with no raw prompt / generated text / PII persisted and no draft or send/message path on the failure branch. See the B-R4 status note above and the contract spec `docs/audits/AREA-B-provider-error-handling.md`. Separately, **PR #128 / `6c829ad`** added a **token / usage cost-guard decision contract** proven over those same recorded usage counts for the current fake-provider scope — a **test-only pure cost-policy helper** (`__tests__/_helpers/ai-runtime-cost-policy.ts`), a **dedicated cost-guard suite** (`__tests__/domains/ai-runtime-cost-guard.test.ts`), and a **vendor-neutral contract doc** (`docs/audits/AREA-B-token-usage-cost-guard.md`) — proving under-budget usage (and usage exactly at the token limit) is allowed and that over-budget / missing / invalid budget or usage all **fail closed** through the `ActionResult` `err(...)` contract, that token counts and ceilings must be **finite non-negative integers** (decimal token counts fail closed), that **`costPerToken` must be > 0** when spend enforcement is configured, that **`maxSpend = 0`** is valid but allows only zero evaluated spend, and that the helper consumes **metadata-only usage counts**. That work added **no real provider, no SDK, no env/API-key read, no schema/migration, no persistent DB usage counters, no route-level wiring, and no auto-send** (`src/domains/ai-runtime/types.ts` untouched — the cost taxonomy lives only in the test helper); real-provider metering, persisted per-business counters, and route-level enforcement remain future / precondition work. See the contract spec `docs/audits/AREA-B-token-usage-cost-guard.md`. Test/doc-only; **no real provider, no route-level wiring, no env/API-key work** added; **real-provider production AI-assisted go-live remains NOT YET APPROVED**.
...
- **Kill switch documented — documentation step DONE via PR #133 / `b1207ae` (2026-06-20).** The kill switch (business-wide, ideally global) must be **documented**: where it lives, how to flip it without a deploy, and the guarantee that flipping it off makes generation **fail closed** (no provider call). This documentation step is now **completed/merged** as the docs-only runbook `docs/operations/ai-runtime-kill-switch-rollback-drill-runbook.md` (PR #133). This satisfies the **documentation** step only: the **execution/rehearsal** of the drill and the **recorded owner sign-off** remain **future work**, so the checkpoint §6 / real-provider-readiness-gate.md gate #11 (rollback / kill-switch drill) is **ADVANCED, NOT CLOSED**.
